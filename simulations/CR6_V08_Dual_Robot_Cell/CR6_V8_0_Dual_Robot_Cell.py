...
def draw_conveyor(ax):
    y0,y1 = CONV_Y_CENTER-CONV_WIDTH/2, CONV_Y_CENTER+CONV_WIDTH/2
    x0,x1,z = CONV_X_START, CONV_X_END, CONV_Z
    bx = np.array([[x0,x1],[x0,x1]])
    by = np.array([[y0,y0],[y1,y1]])
    bz = np.array([[z,z],[z,z]])
    ax.plot_surface(bx,by,bz,color="steelblue",alpha=0.3)
    for yr in [y0,y1]:
        ax.plot([x0,x1],[yr,yr],[z,z],color="dimgray",lw=3)
    for xe in [x0,x1]:
        ax.plot([xe,xe],[y0,y1],[z,z],color="dimgray",lw=3)
    for lx in [x0+0.5,x1-0.5]:
        for ly in [y0,y1]:
            ax.plot([lx,lx],[ly,ly],[0,z],color="dimgray",lw=2)

def draw_station(ax, pos, color, marker):
    hw=0.45
    px,py,pz=pos
    sx=np.array([[px-hw,px+hw],[px-hw,px+hw]])
    sy=np.array([[py-hw,py-hw],[py+hw,py+hw]])
    sz=np.array([[pz-0.08,pz-0.08],[pz-0.08,pz-0.08]])
    ax.plot_surface(sx,sy,sz,color=color,alpha=0.45)
    ax.scatter(px,py,pz,color=color,s=90,marker=marker,zorder=5,
...
def draw_robot(ax, robot):
    ...
    for i in range(len(pts)-1):
        ...
        ax.plot([p1[0],p2[0]],[p1[1],p2[1]],[p1[2],p2[2]],
                color=robot.colors[i],linewidth=lw[i],solid_capstyle="round")
    for pt in pts:
        ax.scatter(*pt,color="white",s=30,zorder=5,edgecolors="gray",linewidths=0.5)
    tcp = pts[-1]
    star_color = "magenta" if robot.name=="A" else "cyan"
    ax.scatter(*tcp,color=star_color,s=130,marker="*",zorder=7)
    robot.trace.append(tcp.copy())
    if len(robot.trace)>2:
        tr=np.array(robot.trace)
        tc="purple" if robot.name=="A" else "teal"
        ax.plot(tr[:,0],tr[:,1],tr[:,2],color=tc,lw=1,alpha=0.3)

def draw_part(ax, part):
    colors = {
        "ON_CONVEYOR":    "darkorange",
        "COMMITTED_TO_A": "darkorange",
        "HELD_BY_A":      "tomato",
        "IN_FIXTURE":     "gold",
        "HELD_BY_B":      "cyan",
        "COMPLETE":       "lime",
    }
    c = colors.get(part.ownership,"white")
    ax.scatter(*part.pos,color=c,s=200,marker="s",
...
def update(frame):
    ...
    ra,rb,p = world.ra, world.rb, world.part

    # Scene
    draw_conveyor(ax)
    draw_station(ax, FIXTURE_POS, "gold",  "D")
    draw_station(ax, OUTPUT_POS,  "lime",  "*")

    # Base rings
    for base in [RA_BASE, RB_BASE]:
        th=np.linspace(0,2*np.pi,32)
        ax.plot(0.55*np.cos(th)+base[0],0.55*np.sin(th)+base[1],
                np.zeros(32),color="gray",lw=2)
    ...
    qb = np.degrees(rb.q)
    hud = (
        f"CHAPPELL ROBOTICS — CR6 V8.0\n"
        f"{'─'*38}\n"
        f"ROBOT A  {('ACTIVE' if ra.active else 'IDLE'):6}  {ra.state}\n"
        f" J1:{qa[0]:+6.1f} J2:{qa[1]:+6.1f} J3:{qa[2]:+6.1f}\n"
        f" J4:{qa[3]:+6.1f} J5:{qa[4]:+6.1f} J6:{qa[5]:+6.1f}\n"
        f"{'─'*38}\n"
        f"ROBOT B  {('ACTIVE' if rb.active else 'IDLE'):6}  {rb.state}\n"
        f" J1:{qb[0]:+6.1f} J2:{qb[1]:+6.1f} J3:{qb[2]:+6.1f}\n"
        f" J4:{qb[3]:+6.1f} J5:{qb[4]:+6.1f} J6:{qb[5]:+6.1f}\n"
        f"{'─'*38}\n"
        f"PART  {p.ownership}\n"
        f" POS [{p.pos[0]:+.2f},{p.pos[1]:+.2f},{p.pos[2]:+.2f}]\n"
        f"{'─'*38}\n"
        f"CLEARED : {world.cleared}\n"
        f"FRAME   : {world.frame}\n"
    )
    ax.text2D(0.02,0.97,hud,transform=ax.transAxes,
              fontsize=7.5,family="monospace",va="top",color="black")

    ax.set_title("CHAPPELL ROBOTICS  —  CR6  V8.0  |  DUAL ROBOT CELL",
                 fontsize=10,fontweight="bold")
    ax.set_xlim(-5,5); ax.set_ylim(-5,5); ax.set_zlim(0,6)
